//////////////////////////////////////////////////////////////////////////////
//
//  Based on syelog of the Microsoft Research Detours Package
//
...
static BOOL syelogIsOpen(PFILETIME pftLog)
{
	...
	s_hPipe = CreateFileW(SYELOG_PIPE_NAMEW,
	...
	if (WaitNamedPipeW(SYELOG_PIPE_NAMEW, 2000)) { // Wait 2 seconds.
...
VOID SyelogOpen(PCSTR pszIdentifier, BYTE nFacility)
{
	...
	s_nProcessId = GetCurrentProcessId();
}
...
VOID SyelogExV(BOOL fTerminate, BYTE nSeverity, PCSTR pszMsgf, va_list args)
{
	...
	*pszEnd = '\0';
	...
	if (syelogIsOpen(&Message.ftOccurance)) {
		if (!WriteFile(s_hPipe, &Message, Message.nBytes, &cbWritten, NULL)) {
			...
			if (s_nPipeError == ERROR_BAD_IMPERSONATION_LEVEL) {
				...
				if (syelogIsOpen(&Message.ftOccurance)) {
					WriteFile(s_hPipe, &Message, Message.nBytes, &cbWritten, NULL);
				}
			}
		}
	}
