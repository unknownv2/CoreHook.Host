...
namespace coreload {
    bool deps_entry_t::to_path(const pal::string_t& base, bool look_in_base, pal::string_t* str) const {
        ...
        if (!exists) {
            logging::logger::instance().verbose(_X("    %s path query did not exist %s"), query_type, candidate.c_str());
            candidate.clear();
        }
        else {
            logging::logger::instance().verbose(_X("    %s path query exists %s"), query_type, candidate.c_str());
        }
        return exists;
    }
